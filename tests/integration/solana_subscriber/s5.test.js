...
describe('S5 — getActiveSubscriptions выбрасывает ошибку', () => {
  ...
  test('если getActiveSubscriptions кидает исключение — логируется и exit(1)', async () => {
    ...
    subscriptions.getActiveSubscriptions.mockRejectedValue(new Error('subs fail'));
    ...
    expect(sharedLogger.sharedLogger).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        level: 'error',
        message: expect.stringContaining('Ошибка при инициализации: subs fail'),
      })
    );
